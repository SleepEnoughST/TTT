...
public class EnemyGo : MonoBehaviour
{
    ...
    private void PlayMoveAnimation()
    {
        ...
        {
            enemyAnimator.SetFloat("走路X", 0);
            enemyAnimator.SetFloat("走路Y", direction);
        }
        else //水平軸向動畫設置
        {
            enemyAnimator.SetFloat("走路X", direction);
            enemyAnimator.SetFloat("走路Y", 0);
        }
    }
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        anim gogogo_13 = collision.gameObject.GetComponent<anim>();
    ...
    public void Fix() //一定得用 public 因為要讓子彈的程式碼調用這個方法
    {
        ...
        audioSource.PlayOneShot(robotHits[RandomNum]);

        Invoke("FixedPlaySound", 1f);
    }
